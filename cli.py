...
def convert(repo: str, out: str = "out_fastapi"):
    """Run the full migration pipeline and write stub output artifacts."""
    _validate_repo_path(repo)
    ...
    typer.echo("[convert] detector:start")
    ...
    typer.echo(f"[convert] detector:complete is_flask_app={flask_detected}")

    if not flask_detected:
        typer.echo("[convert] stopped: target does not appear to be a Flask app")
        raise typer.Exit(code=1)

    typer.echo("[convert] analyzer:start")
    routes = analyze_flask_routes(repo_path)
    _write_json(out_path / "analysis.json", routes)
    typer.echo(f"[convert] analyzer:complete routes={len(routes)}")

    typer.echo("[convert] converter:start")
    conversion_plan = convert_flask_to_fastapi(repo_path, routes)
    _write_json(out_path / "conversion_plan.json", conversion_plan)
    typer.echo(
        "[convert] converter:complete "
        f"generated_files={len(conversion_plan.get('generated_files', {}))}"
    )

    typer.echo("[convert] generator:start")
